...
debug_angle = False
plantime = 10                                                                    # Planning horizon in seconds
...
class JumpPlanner:
    def __init__(self):
        ...
        # ##HORIZON
        self.goal_angles = [[  -3.12,    0.0, 0.0, 0.0],            #Goal roll angles for the planner
                            [  -0.5,     0.0, 0.0, 0.0],            #Goal pitch angles for the planner
                            [   3.09,    0.0, 0.0, 0.0]]           #Goal yaw angles for the planner
        ...
        # Planning parameters
        self.time_step = 0.5                                                          # Time step for FKD model   
    ...
    def evaluate_cost(self, c_states, time_step):
        ...
        vel_cost[velmask] = 3.4 * vel_cost[velmask]**2
        vel_cost[~velmask] = 3.2  * vel_cost[~velmask]**2
    ...
    def execute_cmd(self, action):
        ...
        self.losi.ctrl_cmd['Str_Cmd'] = 1 * np.clip(action[1] + strng,  -1.0 , 1.0) 
        self.losi.send_cmd()
