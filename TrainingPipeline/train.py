...
from model.model import RollPitchAcclnModel  
from model.dataloader import TurntableDataset 
...
class Learner:
    def __init__(self, cfg_dir: str):
        ...
        self.logger.log_code(file_name="model/dataloader.py")
        ...
        self.data, self.val_data, self.dataset_stats = self.init_dataloader()
        ...
        #values for unnormalizing the modle output during physics loss calculation
        self.roll_accln_mean = self.dataset_stats[0]
        self.roll_accln_std = self.dataset_stats[1]
        self.pitch_accln_mean = self.dataset_stats[2]
        self.pitch_accln_std = self.dataset_stats[3]
    ...
    def train(self):
        ...
        rpm = data[0].to(device=self.device)
        rpm_dot = data[1].to(device=self.device)
        steering = data[2].to(device=self.device)
        steering_dot = data[3].to(device=self.device)
        cur_angles = data[4].to(device=self.device)
        angular_vels = data[5].to(device=self.device)

        macs, params = op_counter(self.model, sample=(rpm.unsqueeze(1), rpm_dot.unsqueeze(1), steering.unsqueeze(1), steering_dot.unsqueeze(1), cur_angles, angular_vels))
    ...
    def forward_batch(self, data):
        ...
        rpm_dot = data[1].to(device=self.device).view(-1,1)
        steering = data[2].to(device=self.device).view(-1,1)
        steering_dot = data[3].to(device=self.device).view(-1, 1)
        cur_angles = data[4].to(device=self.device)
        gt_roll_pitch = data[5].to(device=self.device)
        angular_vels = data[6].to(device=self.device)


        roll_pitch_accln_pred_norm = self.model(rpm, rpm_dot, steering, steering_dot, cur_angles, angular_vels)
        loss = self.criterion(roll_pitch_accln_pred_norm, gt_roll_pitch)
    ...
    def validate(self):
        ...
        for data in bar:
            ...
            roll_pitch_accln_pred = self.model(rpm, rpm_dot, steering, steering_dot, cur_angles, angular_vels)
            loss = self.criterion(roll_pitch_accln_pred, gt_roll_pitch)
    ...
    def init_dataloader(self):
        ...
        dataset = TurntableDataset(**self.cfg.dataset)
        dataset_stats = [dataset.roll_accln_mean, dataset.roll_accln_std, dataset.pitch_accln_mean, dataset.pitch_accln_std]
        val_dataset = TurntableDataset(**self.cfg.val_dataset)
        ...
        return data, val_data, dataset_stats
